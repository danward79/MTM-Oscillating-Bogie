...
// count all the data rows. Note that exceedences only can not be counted
      ...
      scatterDimension1    = ndx.dimension(function(d) { return [+d.Time, +d.Speed ]; });
	  scatterDimension2    = ndx.dimension(function(d) { return [+d.Time, +d.Speed ]; });
      scatterGroup1        = scatterDimension1.group().reduceCount(function(d) { return +d.Series === 1; });
	  scatterGroup2        = scatterDimension2.group().reduceCount(function(d) { return +d.Series === 2; });
...
alert(ndx.size());
//alert(scatterDimension.size());  // This return an error
alert(scatterGroup1.size());       // Equals to 4993
alert(scatterGroup2.size());	   // Equals to 4993
//alert(scatterGroup2.groupAll().value());  // This return an error
//alert(ndx.groupAll().value());
var n = scatterDimension1.top(Number.POSITIVE_INFINITY).length;  // This returns 5080
alert(n);
var n2 = scatterGroup1.top(Number.POSITIVE_INFINITY).length;  // This returns 4993
alert(n2);


// Fake group understanding

dim  = ndx.dimension(function(d) { return [+d.Time, +d.Speed ]; });
group = dim.group();

source_group = group.reduce(function(d) {return d['Series'];});

var filtered_groupLHR = doSomething(source_group);

function doSomething(source_group) {
	return {
		all: function () {
...
//alert(filtered_groupLHR.size());  // Does not working
alert(source_group.size());    // return 4993
alert(group.size());
// alert(source_group.groupAll().value());  // returns an error
// alert(group.groupAll().value()); // returns an error
alert(dim.groupAll().value());
...
var chart = dc.compositeChart("#line-chart");
		...
		chart
			...
			.compose([
                ...
                .colors("blue"),
				dc.scatterPlot(chart)
                .group(scatterGroup1, "Exceedences")
                .colors("red"),
			]);		  
